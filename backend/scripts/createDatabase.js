...
if (!fs.existsSync(envPath)) {
    ...
    const defaultEnv = [
        ...
        process.env.NODE_ENV === 'production' ? `PUBLIC_URL=` : null
    ]
    .filter(Boolean)
    .join('\n');
...
async function createDatabase() {
    ...
    if (process.env.NODE_ENV === 'test') {
        ...
        if (testResult.rowCount === 0) {
            logger.info(`Database "${unitTestDbName}" does not exist. Creating...`)
            await client.query(`CREATE DATABASE ${unitTestDbName}`)
            logger.info(`Database "${unitTestDbName}" created.`)
        } else {
            logger.info(`Database "${unitTestDbName}" already exists.`)
        }
        ...
        if (testResult.rowCount === 0) {
            logger.info(`Database "${integrationTestDbName}" does not exist. Creating...`)
            await client.query(`CREATE DATABASE ${integrationTestDbName}`)
            logger.info(`Database "${integrationTestDbName}" created.`)
        } else {
            logger.info(`Database "${integrationTestDbName}" already exists.`)
        }
    } else {
